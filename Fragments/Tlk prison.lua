if not isfile("ZeouronInformation") then
TBOD = false
...
if not game.Workspace:FindFirstChild("Food") then
    SimpleTab.NewPopup("Seems like the server is glitched, ServerHop for a better Zeouron experience", "Middle")
end
...
Misc.NewButton("Change to Prisoner",function()
	...
	for _, connection in ipairs(getconnections(plr.PlayerGui.TeamChangePrisoner.Frame.Yes.MouseButton1Click)) do
	    connection.Function()
	end
end)
...
Remove.NewButton("Remove all Tools",function()
    for i,v in game.Workspace:GetChildren() do if v:IsA("Tool") then game.Players.LocalPlayer.character.Humanoid:EquipTool(v) end end
end)
...
Tools.NewSelector("Get tool",afunction(),function(value)
...
Defensive.NewSwitch("WalkSpeed Bypass",function(bool)
    Wlkbypass = bool
end)

Defensive.NewSlider("WalkSpeed Bypass Speed", {5,300}, 35,function(number)
    Speed = number /100
end)

Defensive.NewSwitch("Auto Sprint",function(bool)
    InfiniteSprint = bool
    while InfiniteSprint and wait(0.01) and lp.character do
...
Defensive.NewSwitch("Autoheal",function(bool)
	...
	SimpleTab.DownValues({"Autoheal: "..tostring(HFIN), "killaura: "..tostring(killaurabool)})
end)
...
Defensive.NewSwitch("Anti Jump",function(bool)
    AntiJump = bool
    if bool then
        while AntiJump and lp.character do
  ...
  game:GetService("RunService").Stepped:Connect(function()
    ...
    if HFIN then
        if plr.character.Humanoid.Health ~= 100 then
    		...
    		local item = lp.character:FindFirstChildWhichIsA("Tool")
      		if item then
            	if item.Name == "Food" or tool.Name == "Hamburger" then
                	if oldtool ~= nil then
    ...
    if DFLO then
        for i,v in pairs(game:GetChildren()) do
            if v.Name == "Flashlight" and v.Handle.Anchored == true then
                flash = v
            end
        end
        fireclickdetector(flash.ClickDetector)
        plr.character.Humanoid:EquipTool(plr.Backpack:FindFirstChild("Flashlight"))
        local tool = plr.character:FindFirstChildWhichIsA("Tool")
        if tool ~= nil then
            tool.Parent = game.Workspace
        end
